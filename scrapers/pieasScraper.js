...
async function scrapPieas() {
  try {
    ...
    for (const { url, name } of pieasUrls) {
      ...
      if (
        dates.thirdTestApplication?.openingDate &&
        dates.thirdTestApplication?.deadlineDate &&
        dates.thirdTestApplication?.lateFeeDeadlineDate
      ) {
      ...
      if (dates.meritNumberAnnouncementDate) {
