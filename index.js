...
async function writeToExcel(
  structuredData,
  fileName = "nust_admissions_data.xlsx"
) {
  const workbook = new ExcelJS.Workbook();
  const sheet = workbook.addWorksheet("NUST UG Admissions");
  ...
  for (const item of structuredData) {
    switch (item.type) {
      ...
      case "table":
        ...
        if (mergeStart !== null && mergeEnd !== null && mergeStart < mergeEnd) {
          sheet.mergeCells(`A${mergeStart}:A${mergeEnd}`);
          sheet.getCell(`A${mergeStart}`).style = centeredCellStyle;
        }
  ...
  console.log(`✅ Excel file created with all content types: ${fileName}`);
}
...
async function main() {
  try {
    const url = "https://nust.edu.pk/admissions/undergraduates/updates-on-ug-admissions/";
