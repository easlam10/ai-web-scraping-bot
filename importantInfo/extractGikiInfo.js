  ...
  for (const para of paragraphs) {
    ...
    if (matches) {
      matches.forEach(match => {
        ...
        extractDatesFromLabelValuePair(label.toLowerCase().trim(), value.trim(), result);
      });
    }
  }
...
function extractDatesFromLabelValuePair(label, value, result) {
  ...
  } else if (label.includes('application deadline') || label.includes('last date to apply')) {
    result.applicationDates = result.applicationDates || {};
    result.applicationDates.deadlineDate = value;
  }

  // Financial aid
  if (label.includes('financial assistance') || label.includes('last date for receipt')) {
  ...
  } else if (label.includes('commencement') || label.includes('classes begin')) {
    result.orientationDates = result.orientationDates || {};
